...
float timed_hash12(vec2 p)
{
	uvec2 q = uvec2(ivec2(p)) * UI2;
	uint n = (q.x ^ q.y) * UI0;
	return fract(dither_shift + (float(n) * UIF));
}
...
float shifted_r_dither(vec2 frag) {
  return fract(dither_shift + (frag.x * 0.75487766624669276 + frag.y * 0.569840290998));
}
...
//   return fract(dither_shift + (float(l + h) * 2.3283064365386963e-10));
// }
