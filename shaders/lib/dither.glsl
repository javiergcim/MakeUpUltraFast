...
float timed_hash12(vec2 p) {
  vec3 p3 = fract(vec3(p.xyx) * .1031);
  p3 += dot(p3, p3.yzx + 33.33);
  return fract(0.2 * frame_mod + ((p3.x + p3.y) * p3.z));
}
...
float shifted_dither_grad_noise(vec2 p) {
  return fract(0.2 * frame_mod + (52.9829189 * fract(0.06711056 * p.x + 0.00583715 * p.y)));
}
...
float shifted_eclectic_dither(vec2 frag) {
  ...
  return fract((0.2 * frame_mod) + p4 + (52.9829189 * fract(0.06711056 * frag.x + 0.00583715 * frag.y)));
}
...
float shifted_texture_noise_64(vec2 p, sampler2D noise) {
  ...
  return fract(0.2 * frame_mod + dither);
}
...
float shifted_phi_noise(uvec2 uv)
{
  ...
  return fract(0.2 * frame_mod + (float(l + h) * 2.3283064365386963e-10));
}
