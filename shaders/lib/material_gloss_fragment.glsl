    ...
    float material_gloss(vec3 reflected_vector, vec2 lmcoord_alt, float gloss_power, vec3 flat_normal) {
        ...
        float astro_vector =
            max(dot(normalize(reflected_vector), normalize(astroLightPos)), 0.0) * step(0.0001, dot(astroLightPos, flat_normal));
    ...
    float material_gloss(vec3 reflected_vector, vec2 lmcoord_alt, float gloss_power, vec3 flat_normal) {
        ...
        float astro_vector =
            max(dot(normalize(reflected_vector), normalize(astroLightPos)), 0.0) *
