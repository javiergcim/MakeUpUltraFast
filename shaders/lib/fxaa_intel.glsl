...
vec3 fxaa311(vec3 color, int iterations){
  ...
  float lumaDown = luma(texture2D(G_COLOR, texcoord.xy + vec2(0.0f,-1.0f) * view, 0.0).rgb);
  float lumaUp = luma(texture2D(G_COLOR, texcoord.xy + vec2(0.0f,1.0f) * view, 0).rgb);
  float lumaLeft = luma(texture2D(G_COLOR, texcoord.xy + vec2(-1.0f,0.0f) * view, 0).rgb);
  float lumaRight = luma(texture2D(G_COLOR, texcoord.xy + vec2(1.0f, 0.0f) * view, 0).rgb);
  ...
  float lumaDownLeft = luma(texture2D(G_COLOR, texcoord.xy + vec2(-1.0f, -1.0f) * view, 0).rgb);
  float lumaUpRight = luma(texture2D(G_COLOR, texcoord.xy + vec2(1.0f, 1.0f) * view, 0).rgb);
  float lumaUpLeft = luma(texture2D(G_COLOR, texcoord.xy + vec2(-1.0f, 1.0f) * view, 0).rgb);
  float lumaDownRight = luma(texture2D(G_COLOR, texcoord.xy + vec2(1.0f, -1.0f) * view, 0).rgb);
  ...
  float lumaEnd1 = luma(texture2D(G_COLOR, uv1, 0).rgb);
  float lumaEnd2 = luma(texture2D(G_COLOR, uv2, 0).rgb);
  ...
  if (!reached1){
  lumaEnd1 = luma(texture2D(G_COLOR, uv1, 0).rgb);
  lumaEnd1 = lumaEnd1 - lumaLocalAverage;
  }
  // If needed, read luma in opposite direction, compute delta.
  if (!reached2){
  lumaEnd2 = luma(texture2D(G_COLOR, uv2, 0).rgb);
  ...
  aa = texture2D(G_COLOR, finalUv, 0).rgb;
  }
  return aa;
}
