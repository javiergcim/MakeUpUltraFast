...
vec3 fxaa311(vec3 color, int iterations){
  ...
  float lumaDown = luma(texture2D(colortex1, texcoord.xy + vec2(0.0f,-1.0f) * view, 0.0).rgb);
  float lumaUp = luma(texture2D(colortex1, texcoord.xy + vec2(0.0f,1.0f) * view, 0).rgb);
  float lumaLeft = luma(texture2D(colortex1, texcoord.xy + vec2(-1.0f,0.0f) * view, 0).rgb);
  float lumaRight = luma(texture2D(colortex1, texcoord.xy + vec2(1.0f, 0.0f) * view, 0).rgb);
  ...
  if (lumaRange > max(edgeThresholdMin, lumaMax * edgeThresholdMax)) {
    ...
    float lumaDownLeft = luma(texture2D(colortex1, texcoord.xy + vec2(-1.0f, -1.0f) * view, 0).rgb);
    float lumaUpRight = luma(texture2D(colortex1, texcoord.xy + vec2(1.0f, 1.0f) * view, 0).rgb);
    float lumaUpLeft = luma(texture2D(colortex1, texcoord.xy + vec2(-1.0f, 1.0f) * view, 0).rgb);
    float lumaDownRight = luma(texture2D(colortex1, texcoord.xy + vec2(1.0f, -1.0f) * view, 0).rgb);
