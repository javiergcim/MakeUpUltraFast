...
vec3 motion_blur(vec3 color, float the_depth, vec2 blur_velocity, sampler2D image, float pixel_size_x, float pixel_size_y, float dither_shift) {
  if (the_depth > 0.7) {  // Mano no
    ...
    #if AA_TYPE > 0
      vec2 coord =
        texcoord - blur_velocity * (1.5 + shifted_r_dither(gl_FragCoord.xy, dither_shift));
    #else
      vec2 coord =
    ...
    for(int i = 0; i < MOTION_BLUR_SAMPLES; i++, coord += blur_velocity) {
      ...
      // b_sample = texture2DLod(image, sample_coord).rgb;
      m_blur += b_sample;
      weight++;
    }
