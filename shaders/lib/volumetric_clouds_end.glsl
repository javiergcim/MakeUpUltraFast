...
vec3 get_end_cloud(vec3 view_vector, vec3 block_color, float bright, float dither, vec3 base_pos, int samples) {
    ...
    block_color.rgb *=
        ...
        float d_dh = texture2DLod(dhDepthTex0, vec2(gl_FragCoord.x / viewWidth, gl_FragCoord.y / viewHeight), 0.0).r;
        ...
        for (int i = 0; i < samples; i++) {
            ...
            if (  // Dentro de la nube
                ...
                if (first_contact) {
                    first_contact = false;
                    density =
                        (surface_sup - intersection_pos.y) / (CLOUD_PLANE_SUP - CLOUD_PLANE);
                }
            }
